...
RTT 是一款面向 Mac 视频观看场景的实时双语字幕工具，适合 YouTube、网课、播客、直播和本地播放器。它捕获系统音频，使用 Apple SpeechAnalyzer 在设备端识别外语语音，再通过 Bing 翻译成简体中文，以置顶悬浮窗显示双语字幕。

## 核心特性

- **系统音频 / 麦克风双来源**：默认捕获系统音频（YouTube、网课、直播、本地播放器），也可切换为麦克风输入，用于会议、口语练习，支持粤语识别。
- **设备端语音识别**：基于 macOS SpeechAnalyzer，识别在本地完成；翻译走 Bing 在线，并内置 Apple Translation 设备端引擎作为可选 / 回退。
- **置顶悬浮窗双语字幕**：原文 / 译文一键切换，窗口锁定防止观看视频时误拖动、误缩放。
- **字幕样式模板**：默认 / 黑底 / 透明 / 学习，适配不同视频背景。
- **低延迟预览**：句子完成前即可看到临时翻译，完成后替换为正式双语字幕。
- **长静音自动断句**：内置 VAD（语音活动检测），长静音时强制断句，避免一句拖得太长。
- **术语表**：自定义关键术语的固定译法，避免专有名词被误翻。
- **转写浏览器**：会话内集中回看本次全部字幕记录。
- **失败重试与复制**：翻译失败可单句重试；可复制当前字幕或最近几条字幕。
- **多格式导出**：双语 SRT / TXT / Markdown。
- **模型与语言包管理**：查看语言模型下载进度，按需释放已用语言包。
- **应用内自动更新**：菜单栏检查更新，默认每天自动检查一次。
...
1. 前往 [Releases](https://github.com/javaht/RTT/releases) 下载最新的 `RTT-x.x.x-universal.dmg`。
2. 打开 DMG，把 RTT 拖入 `Applications`。
3. 首次启动时按系统提示授予**屏幕录制**权限（用于捕获系统音频）；若使用麦克风来源，还需授予麦克风权限。

DMG 默认使用临时签名，首次运行 macOS 可能提示"无法验证开发者"。前往 *系统设置 → 隐私与安全性*，点"仍要打开"即可放行。
...
**语言模型管理**
Apple 语音模型由 macOS 的 `AssetInventory` 统一管理，并可能被 Siri、听写或其他应用共享。"删除语言包"只解除 RTT 对该语言模型的保留；释放成功后模型会从 RTT 的管理列表移除，但 macOS 可能继续保留系统共享文件，RTT 无法强制立即释放对应磁盘空间。
...
- [Homebrew](https://brew.sh/) 安装的 GNU Awk 5.4.1 及其运行库（仅从源码构建时需要）
- 屏幕录制权限，用于捕获系统音频
- 网络连接，用于 Bing 翻译和首次下载语音模型

## 从源码构建

> 以下面向开发者。普通用户直接用上方的 [安装](#安装) 下载 DMG 即可。
...
主要源码位于 `Sources/RTT/`，测试位于 `Tests/RTTTests/`。
...
- 公证（notarization）可消除该警告，属于独立的签名基础设施任务，当前明确不做。
